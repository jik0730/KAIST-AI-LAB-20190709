# code is mainly based on https://github.com/hwalsuklee/tensorflow-mnist-VAE
import numpy as np
...
class Plot_Reproduce_Performance():
    def __init__(self, DIR, n_img_x=10, n_img_y=10, img_w=28, img_h=28, resize_factor=1.0):
        self.DIR = DIR
    ...
    def save_images(self, images, name='result.jpg'):
        ...
        result = Image.fromarray(self._merge(images, [self.n_img_y, self.n_img_x])).convert("L")
        result.save(self.DIR + "/" + name)
    ...
    def _merge(self, images, size):
        ...
        for idx, image in enumerate(images):
            ...
            image = image * 255
            image_ = np.array(Image.fromarray(image).resize((w_, h_)).convert("L"))
...
class Plot_Manifold_Learning_Result():
    ...
    def _set_latent_vectors(self):
        ...
        z = np.rollaxis(
            np.mgrid[self.z_range:-self.z_range:self.n_img_y *
